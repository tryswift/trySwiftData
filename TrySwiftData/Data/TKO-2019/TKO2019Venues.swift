//
//  NYC2016Venues.swift
//  TrySwiftData
//
//  Created by Tim Oliver on 1/29/17.
//  Copyright © 2017 NatashaTheRobot. All rights reserved.
//

import Foundation

public let tko2019Venues: [String : Venue] = [
    "bellesalle-shibuya-first" : Venue(
        title: "Belle Salle Shibuya First",
        titleJP: "ベルサール渋谷ファースト",
        address: "BELLESALLE Shibuya First 1-2-20 Higashi Sumitomo Fudosan Shibuya First Tower B1 Shibuya, Tokyo 150-0011",
        addressJP: "〒150-0011 東京都渋谷区東1-2-20 住友不動産渋谷ファーストタワーB1 ベルサール渋谷ファースト",
        website: "https://www.bellesalle.co.jp/shisetsu/shibuya/bs_shibuyafirst/access/",
        twitter: "https://twitter.com/tryswiftconf",
        logoAssetName: "hallA.jpg",
        wifiNetworkName: "try Swift NYC", //TODO
        wifiUsername: "try Swift NYC", //TODO
        wifiPassword: "SwiftBird18", //TODO
        type: .conference
    ),
    "newworldstages" : Venue(
        title: "New World Stages",
        titleJP: nil,
        address: "340 W 50th St, New York, NY 10019",
        addressJP: nil,
        website: "http://newworldstages.com/",
        twitter: nil,
        logoAssetName: "nws.jpg",
        wifiNetworkName: "try Swift NYC",
        wifiUsername: "try Swift NYC",
        wifiPassword: "SwiftBird18",
        type: .conference
    ),
    "frames" : Venue(
        title: "Frames NYC",
        titleJP: nil,
        address: "550 9th Ave, New York, NY 10018",
        addressJP: nil,
        website: "https://www.framesnyc.com/",
        twitter: nil,
        logoAssetName: "frames.jpg",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .party
    ),
    
    "kitura_workshop" : Venue(
        title: "590 Madison Avenue, 12-1220",
        titleJP: nil,
        address: "590 Madison Avenue, New York, NY 10022",
        addressJP: nil,
        website: nil,
        twitter: nil,
        logoAssetName: "ibmoffice.jpg",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
    "functional_workshop" : Venue(
        title: "33 West 60th Street, 8th Floor",
        titleJP: nil,
        address: "33 West 60th Street, New York, NY 10023",
        addressJP: nil,
        website: nil,
        twitter: nil,
        logoAssetName: "talkspace.png",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
    "vapor_workshop" : Venue(
        title: "345 West 35th Street",
        titleJP: nil,
        address: "345 West 35th Street, New York, NY 10001",
        addressJP: nil,
        website: "wyndhamhotels.com/tryp",
        twitter: nil,
        logoAssetName: "tryp.jpg",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
    "kotlin_workshop" : Venue(
        title: "138 West 25th Street, 7th FL",
        titleJP: nil,
        address: "138 West 25th Street, New York, NY 10001",
        addressJP: nil,
        website: nil,
        twitter: nil,
        logoAssetName: "kotlinvenue.jpg",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
    "ar_workshop" : Venue(
        title: "1441 Broadway, 6th floor",
        titleJP: nil,
        address: "1441 Broadway, New York, NY 10018",
        addressJP: nil,
        website: nil,
        twitter: nil,
        logoAssetName: "jaysuites.jpg",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
    "accessibility_workshop" : Venue(
        title: "20 W 23rd St, 4th floor",
        titleJP: nil,
        address: "20 W 23rd St, New York, NY 10010",
        addressJP: nil,
        website: nil,
        twitter: nil,
        logoAssetName: "brick.jpg",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
    "crypto_workshop" : Venue(
        title: "900 Broadway, Suite 404",
        titleJP: nil,
        address: "900 Broadway, Suite 404, New York, NY 10003",
        addressJP: nil,
        website: nil,
        twitter: nil,
        logoAssetName: "tanooki_square.png",
        wifiNetworkName: nil,
        wifiUsername: nil,
        wifiPassword: nil,
        type: .workshop
    ),
    
]


